  ...
  <div class="py-5">
    <div class="container">
      ...
      <div v-else-if="!request" class="text-center py-5">
        ...
        <h3 class="mt-3">{{ t('clientRequest.notFound') }}</h3>
        <p class="text-muted">{{ t('clientRequest.notFoundHint') }}</p>
        <NuxtLink to="/personal-shopping" class="btn btn-primary">
          {{ t('clientRequest.backToRequests') }}
        </NuxtLink>
      </div>
      ...
      <div v-else>
        ...
        <nav aria-label="breadcrumb" class="mb-4">
          <ol class="breadcrumb">
            <li class="breadcrumb-item"><NuxtLink to="/">{{ t('nav.home') }}</NuxtLink></li>
            <li class="breadcrumb-item"><NuxtLink to="/personal-shopping">{{ t('nav.personalShopping') }}</NuxtLink></li>
        ...
        <div class="row g-4">
          ...
          <div class="col-lg-8">
            ...
            <div v-if="request.items && request.items.length > 0" class="card border-0 shadow-sm mb-4">
              <div class="card-header bg-transparent">
                <h5 class="mb-0"><i class="bi bi-bag-check me-2 text-primary"></i>{{ t('clientRequest.items') }}</h5>
              </div>
                ...
                <div class="table-responsive">
                  <table class="table table-hover mb-0">
                    <thead class="table-light">
                      <tr>
                        <th>{{ t('clientRequest.product') }}</th>
                        <th>{{ t('clientRequest.unitPrice') }}</th>
                        <th>{{ t('clientRequest.qty') }}</th>
                        <th class="text-end">{{ t('clientRequest.total') }}</th>
                      </tr>
                    </thead>
                    <tbody>
                    ...
                    <tfoot class="table-light">
                      <tr>
                        <td colspan="3" class="fw-bold">{{ t('clientRequest.estimatedTotal') }}</td>
            ...
            <div class="card border-0 shadow-sm mb-4">
              <div class="card-body p-4">
                ...
                <div v-else class="text-center py-5 bg-light rounded">
                  ...
                  <p class="text-muted mt-2">{{ t('clientRequest.noImage') }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
              <div class="card-header bg-transparent">
                <h5 class="mb-0">{{ t('clientRequest.details') }}</h5>
              </div>
              <div class="card-body">
                ...
                <div class="row g-3 mt-3 stat-row">
                  <div class="col-6 col-md-3">
                    <div class="p-3 bg-light rounded">
                      <small class="text-muted d-block">{{ t('clientRequest.quantity') }}</small>
                      <strong>{{ request.quantity }}</strong>
                    </div>
                  </div>
                  <div class="col-6 col-md-3">
                    <div class="p-3 bg-light rounded">
                      <small class="text-muted d-block">{{ t('clientRequest.estimatedBudget') }}</small>
                      <strong>{{ formatCurrency(request.budgetEstimated, requestCurrency) }}</strong>
                    </div>
                  </div>
                  <div class="col-6 col-md-3">
                    <div class="p-3 bg-light rounded">
                      <small class="text-muted d-block">{{ t('clientRequest.createdOn') }}</small>
                      <strong>{{ formatDateShort(request.createdAt) }}</strong>
                    </div>
                  </div>
                  <div class="col-6 col-md-3">
                    <div class="p-3 bg-light rounded">
                      <small class="text-muted d-block">{{ t('clientRequest.messages') }}</small>
            ...
            <div v-if="request.quotedPrice" class="card border-0 shadow-sm mb-4 border-success">
              <div class="card-header bg-success-subtle">
                <h5 class="mb-0 text-success"><i class="bi bi-receipt me-2"></i>{{ t('clientRequest.quote') }}</h5>
              </div>
              <div class="card-body">
                <div class="table-responsive">
                ...
                <div v-if="request.status === 'negotiating'" class="d-grid gap-2 mt-4">
                  <button class="btn btn-success btn-lg" @click="acceptQuotation">
                    <i class="bi bi-check-circle me-2"></i>{{ t('clientRequest.acceptQuote') }}
                  </button>
                  <button class="btn btn-outline-secondary" @click="openWhatsApp">
                    <i class="bi bi-whatsapp me-2"></i>{{ t('clientRequest.negotiate') }}
                  </button>
                </div>
                ...
                <div v-if="(request.quotedPrice || 0) > 0 && ['negotiating', 'confirmed'].includes(request.status)" class="mt-3">
                  <div class="d-flex justify-content-between align-items-center bg-light rounded p-2 mb-2 small">
                    <span class="text-muted">{{ t('clientRequest.amountDue') }} <span class="text-muted">{{ t('clientRequest.feesIncluded') }}</span></span>
                  ...
                  <div class="d-grid gap-2">
                    <button class="btn btn-primary btn-lg" :disabled="payProcessing !== null" @click="pay('personal_shopping', String(request.id))">
                      ...
                      {{ payProcessing !== null ? t('clientRequest.redirecting') : t('clientRequest.payNow') }}
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div v-if="request.trackingNumber" class="card border-0 shadow-sm">
              <div class="card-header bg-transparent">
                <h5 class="mb-0"><i class="bi bi-truck me-2"></i>{{ t('clientRequest.shipmentTracking') }}</h5>
              </div>
              <div class="card-body">
                <p class="mb-2">
                  <strong>{{ t('clientRequest.trackingNumber') }}</strong>
          ...
          <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm mb-4">
              <div class="card-body">
                <h6 class="fw-medium mb-3">{{ t('clientRequest.requestStatus') }}</h6>
            ...
            <div class="card border-0 shadow-sm mb-4">
              <div class="card-body">
                <h6 class="fw-medium mb-3">{{ t('clientRequest.actions') }}</h6>
                <div class="d-grid gap-2">
                  <button class="btn btn-warning position-relative" @click="addRequestToCart">
                    <i class="bi bi-cart-plus me-2"></i>{{ t('clientRequest.addToCart') }}
                  </button>
                  <a
                    href="#"
                  ...
                  >
                    <i class="bi bi-whatsapp me-2"></i>{{ t('clientRequest.contactWhatsapp') }}
                  </a>
                  <NuxtLink to="/personal-shopping/new" class="btn btn-outline-primary">
                    <i class="bi bi-plus-circle me-2"></i>{{ t('clientRequest.newRequest') }}
                  </NuxtLink>
            ...
            <div class="card border-0 bg-light">
              <div class="card-body">
                <h6 class="fw-medium mb-2"><i class="bi bi-question-circle me-2"></i>{{ t('clientRequest.needHelp') }}</h6>
                <p class="text-muted small mb-2">
                  {{ t('clientRequest.helpTeam') }}
                </p>
                <NuxtLink to="/resources/faq" class="btn btn-link btn-sm p-0">
                  {{ t('clientRequest.viewFaq') }}
                </NuxtLink>
              </div>
            </div>
          </div>
...
function itemName(item: { name_fr?: string; name_en?: string; name_zh?: string }) {
  const current = String(locale.value || '')
  if (current.startsWith('zh')) return item.name_zh || item.name_en || item.name_fr || t('clientRequest.product')
  if (current.startsWith('en')) return item.name_en || item.name_fr || item.name_zh || t('clientRequest.product')
  return item.name_fr || item.name_en || item.name_zh || t('clientRequest.product')
}

const statusTimeline = computed(() => [
  { key: 'pending', label: t('personalShopping.status.pending') },
  { key: 'searching', label: t('personalShopping.status.searching') },
  { key: 'negotiating', label: t('personalShopping.status.negotiating') },
  { key: 'confirmed', label: t('personalShopping.status.confirmed') },
  { key: 'preparing', label: t('personalShopping.status.preparing') },
  { key: 'shipped', label: t('personalShopping.status.shipped') },
  { key: 'delivered', label: t('personalShopping.status.delivered') },
])
...
const acceptQuotation = async () => {
  if (request.value) {
    ...
    success(t('clientRequest.quoteAccepted'))
  }
}
